...
from data.CF_data import (
    save_CF, read_CF, read_Map,
    save_gamecomplete, credits as CF_data_credits
)
from entities import Map, CF
from logic.pygame.CF_object_oldver import(
    Player,
...
class Start_Map():
    def __init__(self,
        ...
        '''
        current_map = Map
        read_Map( Map=current_map, level=level )

        # Establecer archivo y información del level
        next_level = [current_map.path, current_map.next_level]
        self.level = level.replace(dir_maps, '')

        self.climate = current_map.climate
        ...
        map_level = current_map.list_map
...
while exec_game:
    ...
    if credits == False:
        ...
        if isinstance(message_start, str):

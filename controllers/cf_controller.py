...
from core.file_util import(
    get_path, # No se necsita get path
    list_files
)
from core.text_util import(
    read_text,
    ignore_comment, separe_text, only_the_comment, ignore_text_filter
)

from entities import CF, Map
from models import CuadradoFelizConfig, CuadradoFelizMap # Aun no se usa
...
dir_game = resource_loader.base_dir

dir_data = resource_loader.resources_dir

# Subcarpetas
dir_maps = os.path.join(dir_data, 'maps')

# Archivo de data
dir_game_data = dir_data.joinpath('CF.dat')

# Archivo de juego completado
file_gamecomplete = dir_data.joinpath('gamecomplete.dat')


def get_data(mode_dict=True):
    '''Obtener los datos de ejecución del juego'''
    # Leer texto y ignorar comentarios tipo '#'
    archive_cf = read_text(
        dir_game_data,
        'ModeText'
    )
...
def read_Map( Map, level=str ) -> bool:
    ...
    # Leer archivo
    text_level = read_text(level, 'ModeText')
